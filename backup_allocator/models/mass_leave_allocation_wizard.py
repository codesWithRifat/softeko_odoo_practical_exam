...
class MassLeaveAllocationWizard(models.TransientModel):
    ...
    allocation_log_ids = fields.One2many(
    comodel_name='mass.leave.allocation.log',
    inverse_name=None,
    string='Logs',
    compute='_compute_allocation_logs',
    store=False
)
    @api.depends()
    def _compute_allocation_logs(self):
        for wizard in self:
            wizard.allocation_log_ids = self.env['mass.leave.allocation.log'].search([
...
class MassLeaveAllocationLog(models.Model):
    ...
    # wizard_id = fields.Many2one('mass.leave.allocation.wizard', string='Wizard', ondelete='cascade')  # ✅ FIXED
